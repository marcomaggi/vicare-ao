...
(parametrise ((check-test-name		'driver-info))
  ...
  (check-for-true
   (non-negative-fixnum? (ao::ao-driver-id "alsa")))

  (check-for-true
   (non-negative-fixnum? (ao::ao-driver-id "oss")))

;;; --------------------------------------------------------------------

  (debug-print (ao::ao-default-driver-id))

  (check-for-true
   (non-negative-fixnum? (ao::ao-default-driver-id)))
